...
mod mft_tests {
    ...
    async fn test_mft_scan_fallback() {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            match result {
                Ok(scan_result) => {
                    println!(
                        "MFT scan succeeded: {} files found in {}ms",
        ...
        #[cfg(not(target_os = "windows"))]
        {
            ...
            match result {
                Ok(scan_result) => {
                    println!(
                        "Unix scan succeeded: {} files found",
    ...
    fn test_mft_record_structures() {
        ...
        let parsed: MftScanResult =
            serde_json::from_str(&json).expect("Failed to deserialize MftScanResult");
    ...
    async fn test_windows_api_structures() {
        ...
        println!(
            "NtfsVolumeData size: {} bytes",
            mem::size_of::<NtfsVolumeData>()
        );
        println!(
            "FileRecordInput size: {} bytes",
            mem::size_of::<FileRecordInput>()
        );
        println!(
            "UsnJournalData size: {} bytes",
            mem::size_of::<UsnJournalData>()
        );
