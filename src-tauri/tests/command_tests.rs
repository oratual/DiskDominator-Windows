...
mod command_tests {
    ...
    async fn test_system_overview() {
        ...
        match result {
            Ok(overview) => {
                ...
                println!("Total space: {} GB", overview.total_space / (1024_u64.pow(3)));
                
                // Basic validation
                assert!(overview.disks.len() > 0, "Should detect at least one disk");
                assert!(overview.total_space > 0, "Total space should be positive");
                assert!(overview.used_space <= overview.total_space, "Used space shouldn't exceed total");
            },
            Err(e) => {
        ...
        match result {
            Ok(activities) => {
                ...
                for activity in &activities {
                    ...
                    assert!(!activity.action.is_empty(), "Activity action should not be empty");
                    assert!(!activity.target.is_empty(), "Activity target should not be empty");
                }
            },
            Err(e) => {
    ...
    fn test_activity_type_serialization() {
        ...
        let parsed: ActivityType = serde_json::from_str(&json).expect("Failed to deserialize ActivityType");
    ...
    fn test_scan_type_conversion() {
        ...
        // Test serialization
        let json_quick = serde_json::to_string(&quick).expect("Failed to serialize DiskScanType::Quick");
        let json_deep = serde_json::to_string(&deep).expect("Failed to serialize DiskScanType::Deep");
        let json_custom = serde_json::to_string(&custom).expect("Failed to serialize DiskScanType::Custom");
